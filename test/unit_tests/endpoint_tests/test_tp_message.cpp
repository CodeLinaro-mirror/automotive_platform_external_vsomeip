...
void write_uint32_be(std::vector<byte_t>& _buffer, std::size_t _pos, std::uint32_t _value) {
...
std::vector<byte_t> make_segment(std::uint32_t _offset, bool _more_segments, std::uint32_t _payload_size, byte_t _fill) {
    ...
    write_uint32_be(its_datagram, VSOMEIP_LENGTH_POS_MIN, static_cast<std::uint32_t>(its_datagram.size()) - VSOMEIP_SOMEIP_HEADER_SIZE);
...
TEST(tp_message_test, overlapping_segment_with_gap_does_not_over_read) {
    ...
    tp_message its_message(its_segment_a.data(), static_cast<std::uint32_t>(its_segment_a.size()), max_message_size);

    // segment C -> [8388608, 8388623], far behind A so that a gap remains in between
    constexpr std::uint32_t far_offset = 8 * 1024 * 1024;
    const auto its_segment_c = make_segment(far_offset, true, 16, 0xC3);
    EXPECT_FALSE(its_message.add_segment(its_segment_c.data(), static_cast<std::uint32_t>(its_segment_c.size())));
    ...
    EXPECT_FALSE(its_message.add_segment(its_segment_b.data(), static_cast<std::uint32_t>(its_segment_b.size())));
}
...
TEST(tp_message_test, overlapping_segment_with_gap_reassembles_correctly) {
    ...
    EXPECT_FALSE(its_message.add_segment(its_segment_c.data(), static_cast<std::uint32_t>(its_segment_c.size())));
    EXPECT_FALSE(its_message.add_segment(its_segment_b.data(), static_cast<std::uint32_t>(its_segment_b.size())));
    ASSERT_TRUE(its_message.add_segment(its_segment_d.data(), static_cast<std::uint32_t>(its_segment_d.size())));
...
TEST(tp_message_test, zero_length_segment_is_rejected) {
    ...
    tp_message its_message(its_first.data(), static_cast<std::uint32_t>(its_first.size()), max_message_size);
    ...
    ASSERT_EQ(its_zero_length.size(), static_cast<std::size_t>(VSOMEIP_TP_PAYLOAD_POS));
    EXPECT_FALSE(its_message.add_segment(its_zero_length.data(), static_cast<std::uint32_t>(its_zero_length.size())));
}
...
TEST(tp_message_test, segment_after_headerless_first_datagram_is_rejected) {
    ...
    tp_message its_message(its_truncated.data(), static_cast<std::uint32_t>(its_truncated.size()), max_message_size);
    ...
    EXPECT_FALSE(its_message.add_segment(its_last.data(), static_cast<std::uint32_t>(its_last.size())));
}
